...
def rd_block(p_reg,p_address):
    ...
    rd_str.append("%12s%s : prdata <= %-10s;\n"%('',p_address,p_reg.upper()))
    return rd_str
...
#######################20200722##########################
    ...
    #main logic Rd
    fo.write("always@(posrdge clk or negedge rst_n) begin\n")
    fo.write("    if(rst_n == 1'b0)begin\n")
    fo.write("        prdata <= 32'h0\n")	
    fo.write("    end \n")	
    fo.write("    else if(reg_rd)begin\n")	
    fo.write("        case(paddr)\n")
    ...
    for row in range (p_sheet.nrows)[1:]:
        ...
        if(p_sheet.cell(row,adr_col).value!=''):
            ...
            fo.write("".join(rd_block(reg_name,address)))
